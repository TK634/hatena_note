...
毎朝9:00 (launchd)
  └─ run.sh → run-all.ts
       ...
       ├─ analyze.ts        … 全記事の反応データを収集し勝ちテーマを更新（自己改善ループ）
       ├─ ジャンルごとに:
       │    ├─ generate.ts  … Claude Haiku で記事生成（予算ガード付き）
       │    │                  3日に1回は「勝ちテーマの深掘り記事」を自動生成
...
毎朝の実行時にデータ収集→勝ちテーマ特定→翌日以降の生成に反映、を自動で回す。

1. **収集**: `analyze.ts` が全記事のはてなブックマーク数を取得（無料API・課金なし）。
   `manual-signals.json` に Search Console のクリック数を手で足すと精度が上がる（任意）。
2. **特定**: スコア上位記事を `winning-topics.json` に保存。`performance-log.json` に推移を蓄積。
3. **反映**: 偏りすぎによる機会損失を防ぐため、3日サイクルでバランスを取る:
   - **深掘り日**: 反応が良かった自記事の関連テーマを生成（活用）
   - **トレンド日**: はてブ人気エントリ（`trend-topics.ts`）から「読まれるタイトルの型・切り口」を
     学び、ジャンルに応用したテーマを生成（探索）。内容のコピーは禁止、学ぶのは型のみ
   - **通常日**: トピックローテーション（網羅性の維持）
   ※3ジャンルは毎日すべて投稿されるため、ジャンル間の偏りは発生しない
4. **リンク改廃**: 提携承認され次第 `affiliate-links.ts` の PENDING を差し替えるだけで全記事に反映。
   記事末尾のアフィリリンクは、AIが記事テーマに直接関係するものだけを選定（無関係リンク排除）。
